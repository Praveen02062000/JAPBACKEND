...
function ExcelCovertMethod(data) {
    try {
        ...
        ExcelCovert.writeFile(workbook, './Routers/ExcelFile/Studentdata.xlsx');
...
function ConvertDataFormat(data, batch_id) {
    try {
        if (data.length > 0) {
            let finalConvertedData = [];
            let studentNameFilterSqlQuery = `select name,id from Student where batch_id = "${batch_id}"`;
            let final = DB.all(studentNameFilterSqlQuery, [], (err, Studentdata) => {
                if (err) {
                    ...
                    if (Studentdata.length > 0) {
                        ...
                        let studentobjInterface = { Classdate: null };
                        for (const std of Studentdata) {
                            studentobjInterface[std.name] = null;
                        }

                        let DateVsData = {};
                        for (const x of data) {
                            DateVsData[x.classdate] = []
                        }

                        for (const keys of Object.keys(DateVsData)) {
                            for (const attenddata of data) {
                                if (attenddata.classdate === keys) {
                                    DateVsData[keys].push({ std_id: attenddata.student_id, action: attenddata.action_attend });
                                }
                            }
                        }


                        for (const keydate of Object.keys(DateVsData)) {
                            let newstdobj = { ...studentobjInterface };
                            newstdobj["Classdate"] = keydate;
                            for (const stddata of DateVsData[keydate]) {
                                for (const orignalstd of Studentdata) {
                                    if (orignalstd.id === stddata.std_id) {
                                        newstdobj[orignalstd.name] = stddata.action === 'true' ? 'Present' : 'Absent'
                                    }
                                }
                            }
                            finalConvertedData.push(newstdobj)
                        }


                        ExcelCovertMethod(finalConvertedData)
    ...
    try {
        ...
        DB.all(sqlquery, [], (err, row) => {
            if (err) {
                ...
                // console.log(row)
                ConvertDataFormat(row, batch_id);
